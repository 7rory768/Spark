...
END;");
            ...
            procedures.Add(@"DROP PROCEDURE IF EXISTS `getAssignedToTask`; CREATE PROCEDURE IF NOT EXISTS PROCEDURE `getAssignedToTask`(IN _taskId INT)
BEGIN
	
	SELECT users.* FROM users INNER JOIN assigned_to ON (users.username = assigned_to.username) WHERE assigned_to.taskId=_taskId;
...
END;");
            ...
            procedures.Add(@"DROP PROCEDURE IF EXISTS `getChecklistItems`;
CREATE PROCEDURE IF NOT EXISTS `getChecklistItems`(IN _checklistId INT)
BEGIN
	
	SELECT * FROM checklist_items WHERE checklistId=_checklistId;
	
END;");

            procedures.Add(@"DROP PROCEDURE IF EXISTS `getChecklists`;
CREATE PROCEDURE IF NOT EXISTS `getChecklists`(IN _taskId INT)
BEGIN
	
	SELECT * FROM checklists WHERE taskId=_taskId;

END;");

            procedures.Add(@"DROP PROCEDURE IF EXISTS `createChecklist`;
CREATE PROCEDURE IF NOT EXISTS `createChecklist`(IN `_taskId` INT, IN title VARCHAR(255))
BEGIN
